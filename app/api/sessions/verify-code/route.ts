...
// 옛 방침 "검사한 번호 목록은 반환하지 않는다"(사용자 확정 2026-08-13 — 시작 화면은 아동
// 앞의 PC이고 학급 안에서 번호는 사실상 이름이라는 근거)는 명단 드롭다운 도입으로
// 뒤집혔다(사용자 확정 2026-08-18 — 명단 드롭다운이 스펙에 포함되며 뒤집기가 그로부터
// 필연적으로 따라 나온다) — childNo 없이 부르면 명단 전체(번호별 검사 상태 포함)를 돌려준다.
