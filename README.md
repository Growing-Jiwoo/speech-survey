...
학교·학년·반·담임·연락처는 관리자가 코드 발급 시 입력하거나(`/admin/codes`), 교사가 `/apply`에서
직접 신청해 채워진다 — 어느 경로든 **한 번만 입력**한다. 세션 생성 시 서버가 코드를 다시 조회해 그
학급 정보를 `sessions`에 복사한다(비정규화: 코드를 나중에 고쳐도 이미 만든 세션은 검사 당시 값을
유지). 진행 상태는 localStorage에 저장돼 새로고침·탭 닫힘 후에도 같은 문항에서 재개된다.
