...
검사지가 학년마다 다르다. 세션의 학년이 양식을 정하고(`formForGrade`), 양식이 문항·배점·중단
규칙·인쇄 좌표를 전부 결정한다.
...
| 쓰기 중단 규칙 | **1번 낱말** 오반응 | **첫 문장** 오반응(0점) |
...
⚠️ **단 하나의 예외 — 중단 규칙.** 검사 내용물(문항·낱말·배점)은 위 원칙 그대로지만, **시행 절차**는
담당자 회신이 우선한다. 2026-08-11 확정으로 중단 규칙이 인쇄 문구와 세 지점에서 다르다:
무의미 낱말 해독은 **실시하지 않고**(문구엔 없음), 쓰기 과제는 **실시하며**(문구는 미실시),
G1 쓰기 중단은 "의미 낱말 첫 3개"가 아니라 **1번 낱말 하나**로 판정한다.
`lib/survey-flow.ts`가 검사지와 달라 보이는 것은 버그가 아니다 — 되돌리기 전에
...
2. Supabase 프로젝트(리전: 서울/ap-northeast-2) 생성 → SQL Editor에서 순서대로 실행:
   `001_init.sql` → `003_kodys_redesign.sql` → `004_login_attempts.sql` → `005_cascade_and_indexes.sql` → `006_login_lockout_decay.sql` → `007_harden_rpc.sql` → `008_guardian_consent.sql` → `009_reading_marks.sql` → `010_class_and_contact.sql` → `011_scoring.sql` → `012_discontinued.sql`
   ...
   - `009`는 낱말 해독 현장 채점(O/X) 테이블, `010`은 단일학급(반 0)·담임 연락처 분리,
     `011`은 문장 어절 점수 테이블(`sentence_scores`)과 검사자 구분, `012`는 중단 규칙 적용 시각 컬럼.
   - `005`~`012`는 비파괴적이며 재실행해도 안전하다.
...
- [ ] 관리자: 로그인 → 목록(검색·필터) → 결과지(녹음 청취·쓰기·체크리스트) → 검사지 PDF 다운로드(학년별 양식 확인) → 세션 삭제(확인 모달) → 로그아웃
