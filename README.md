...
2. Supabase 프로젝트(리전: 서울/ap-northeast-2) 생성 → SQL Editor에서 순서대로 실행:
   ...
   - `005`는 공개 엔드포인트(세션 생성·녹음 업로드) IP 남용 방지용 `rate_limits` 테이블을 만든다.
3. `cp .env.local.example .env.local` 후 값 채우기
   - `ADMIN_PASSWORD_HASH`: `npm run hash-password -- '원하는비밀번호'` 출력값
     (salted scrypt, `salt-hex:hash-hex` 형식 — 평문 비번 자체를 복구할 순 없지만 강한 비번을 쓸 것)
...
3. **환경변수 등록** (Settings → Environment Variables, Production):
   ...
   - `ADMIN_PASSWORD_HASH` — ⚠️ **로컬에서 쓰던 값이나 예시용 약한 비번 금지.** `npm run hash-password -- '강한랜덤비번'` 출력값을 넣는다.
