...
## 미실시 · 미녹음의 표기 (혼동하기 쉬운 두 가지)

| 상황 | 표기 | 근거 |
|---|---|---|
| 중단 규칙으로 **실시하지 않은** 과제·문항 | 점수를 적지 않는다(화면 `미실시`·`중단`, PDF 빈칸) | 치르지 않은 과제에서 낙제한 아동으로 기록되면 안 된다 |
...
**사후 중단** — 관리자가 녹음을 듣고 의미 낱말을 고쳐 뒤늦게 규칙 ①이 성립하면, 검사 당시엔
실시된 무의미 낱말·문장 읽기유창성의 점수가 이미 저장돼 있다. 그 값은 **지우지 않고 채점에서만
빼며**(화면·PDF 모두 `미실시`), 잠긴 입력칸에 회색으로 남겨 오채점인지 확인할 수 있게 한다.
의미 낱말 O/X를 되돌리면 점수가 그대로 되살아난다(파생값이라 자동). 이 수정으로 중단이 새로
성립하는 순간에는 결과지가 확인 모달을 띄운다. (쓰기 과제는 ①에서도 실시하므로 제외 대상이 아니다.)

중단 이후 문항의 답은 **저장하지 않는다.** 검사자가 뒤 문항을 먼저 채점하고 1번을 마지막에
오반응으로 찍는 경우가 있어(실제 사례), 화면은 중단 안내 확인 시점에, 서버는 제출 시점에
`keepImplementedWriting`으로 절삭한다. 이미 그렇게 저장된 세션은 읽는 쪽(채점·진행률·PDF)에서
같은 판정으로 제외한다.
...
- [ ] 쓰기 1번을 마지막에 오반응으로 찍어 중단 → 앞서 표시한 2번 이후 값이 저장·표시되지 않는지(진행률 1 / 1)
