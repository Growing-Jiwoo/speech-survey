 ...
 *
 * 업데이트 0건인데 행이 여전히 pending이면(동시 트랜잭션·RLS 등) 던진다 — 승인되지 않은
 * 코드를 already:true로 보고하면 라우트가 메일을 건너뛰어, 교사는 코드를 못 받고 관리자는
 * 보냈다고 생각한다. 실패로 알려 다시 누르게 하는 편이 안전하다(재시도는 사람이 한다).
 */
