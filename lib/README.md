...
| `survey-flow.ts` | 페이지 게이팅. 중단 규칙(①·②)과 낱말 해독 현장 채점이 전면 폐기됐다 — **근거가 규칙마다 다르다**: 현장 채점 제거는 담당자 확정(2026-08-13), 규칙 ①은 담당자 회신이 근거, 규칙 ②(쓰기 중단)는 **담당자 확정(2026-08-22)**이다 — 2026-08-13에는 사용자 확정이라 재확인 대상이었으나 담당자가 「즉시 중단 기능 제거」가 ②를 포함한다고 확정했다(파일 상단 주석 참고). 낱말 해독·문장 읽기유창성은 검사 화면이 수집만 하고 판정·채점은 관리자 결과지가 한다. 다만 쓰기(낱말 쓰기·문장 쓰기)는 예외로, 녹음이 없어 검사 중 검사자가 계속 입력한다(사용자 확정 2026-08-13, `WritingPage.tsx`·`SentenceWritingPage.tsx`). `visiblePages`(연습 실시 여부만 반영)·`canAdvance`(문항 완료 여부로 [다음] 활성화 판단) |
...
| `schema.ts` | 세션 생성 입력의 zod 스키마 — **검증 규칙의 단일 소스**(서버 라우트가 사용). `sessionCreateSchema`는 `sessionCreateFromRosterSchema`(명단 모드, 이름·생년월일 필드 자체가 없다)와 `sessionCreateDirectSchema`(직접 입력, 기존 `SessionCreateInput` 타입)의 `z.union`(Task 13) — 명단 스키마를 먼저 시도하도록 순서를 둬 `fromRoster:true`에 신원 필드를 함께 보내도 그 필드를 조용히 벗기지만, 신원 위조 방지 자체는 순서에 기대지 않는다: `sessionCreateDirectSchema`가 `fromRoster: z.undefined().optional()`을 가져 `fromRoster:true`인 바디는 유니온을 뒤집어도 direct 스키마를 통과할 수 없다(코드리뷰 후속, 2026-08-18). 학급 코드 알파벳(`CODE_ALPHABET`)·길이(`CODE_LEN`)도 여기가 단일 소스 — 발급(`class-code.ts`)과 입력 검증이 공유. 학급 코드 공통 필드는 `classCodeFields`로 분리해 관리자 직접 발급(`classCodeCreateSchema`)과 교사 신청(`applySchema`)이 공유한다 — `.refine()`이 걸리면 `.extend()`가 안 되기 때문. 이메일은 이제 **직접 발급도 필수**다(사용자 확정 2026-09-22) — 결과지 링크가 `teacher_email`로만 가므로 이메일 없는 학급은 발급 경로와 무관하게 결과를 받을 방법이 없어져, 종전 「전화·이메일 둘 중 하나」와 `applySchema`의 대비가 사라졌다. `classCodeEmailSchema`(담임 이메일만 받는 관리자 수정 바디)와 `resultsRequestSchema`(교사 결과지 링크 요청, 코드만 받음)도 여기 |
...
| `db.ts` | DB/스토리지 접근 함수 전부(세션 생성·녹음 기록·제출·삭제·로그인 레이트리밋·관리자 조회·학급 코드 발급/목록/삭제/조회/중복검사 상태). 코드 조회 계열은 `status`(`pending`/`active`)·`applied_at`을 함께 돌려준다 — 승인 전 코드로 검사가 시작되지 않게 호출부가 판단할 근거. 교사 신청 접수는 `insertApplication`(pending 코드 + 명단, `'duplicate'`면 호출부가 새 코드로 재시도) — 부분 삽입을 되돌리는 방식과 그 이유는 함수 docblock이 갖는다. 승인은 `approveClassCode`(pending → active) — `.eq('status','pending')` 한 방으로 멱등을 판정해 `already:true`를 돌려주고, 라우트는 그때 승인 메일을 건너뛴다(더블클릭 재전송 방지). 0건인데 행이 아직 pending이면 던진다 — 승인 안 된 코드를 already로 보고하면 교사가 코드를 못 받는다. 승인 화면이 검토하는 명단은 `listRoster`(번호 순 고정, 읽기 전용). 검사 시작 화면의 드롭다운은
`rosterWithTested`(명단 + 번호별 검사 상태) — `childTestState`와 판정 기준(제출본이 있으면
`submitted`가 항상 이김, 없으면 미제출 존재 여부로 `inProgress`/`null`)이 같아야 하며, 그
근거는 함수 docblock이 갖는다. **`submitSession`은 쓰기 답을 먼저 넣고 `submitted_at`을 마지막에 확정한다 — 순서를 되돌리면 중간 실패 시 재시도가 409로 막혀 쓰기 점수가 영구 유실된다**(쓰기는 검사 중 입력이 유일한 채점 경로다). `createSession`은 `idemKey`가 오면 **같은 키로 세션을 하나만 만든다** — insert를 먼저 던지고 unique 충돌(23505)을 "이미 만든 그 세션"으로 해석한다(먼저 조회하면 경쟁에서 둘 다 만든다). 이유 전문은 함수 docblock. 교사 결과지용으로 `findClassCodeById`(id로 조회, 결과지 토큰이 담는 주체가 id), `classResults`(학급 세션 전부를 채점 4테이블까지 관계 select 한 번으로 — 세션당 4번이면 25명 반에서 100회), `updateClassCodeEmail`(담임 이메일 한 컬럼만 고치는 유일한 경로 — 결과지 링크가 그 주소로만 간다)을 추가했다 |
...
| `consent.ts` | 고지·동의 문구의 단일 소스. 법정대리인 동의 확인 기록(개인정보보호법 제22조의2)의 근거 문구 + `RETENTION_LABEL`(보관 기간 — 시작 화면과 신청 화면이 공유해야 하는 값) + 신청 화면(`/apply`)의 `SURVEY_NOTICE`·`APPLY_CHECKS`. 신청 화면 문구가 화면이 아니라 여기 있는 이유는 ①시작 화면 고지 카드와 같은 값을 써야 하고 ②node 환경 테스트가 문구 존재를 핀할 수 있어야 하기 때문이다(화면 렌더 테스트는 두지 않는 관례) |
| `mail.ts` | 메일 발송 단일 창구 + 문구(신청 알림·승인 안내·결과지 링크). Resend HTTP API를 `fetch`로 직접 부른다(SDK 미사용). `MAIL_TO_OVERRIDE`가 있으면 수신자를 그 주소로 강제해 실수 발송을 막는다. 문구는 사용자 확정 2026-08-22 — 승인 안내는 **코드 전달이 목적**이라 짧게 유지한다(코드·학급·주소·시작 3단계·보관 부탁). 소요 시간·준비물·보호자 동의 조건은 **일부러 담지 않는다** — 신청 화면에서 교사가 이미 읽고 체크한 것이라 중복이었다(실제 발송본 검수 후 제거). 예외로 「결과지 받는 방법」은 담는다(사용자 확정 2026-09-22) — 교사가 결과를 받을 유일한 경로이고 신청 화면에는 없던 정보라 중복이 아니다. 문구는 `format.ts`의 `RESULTS_GUIDE_LINES` 하나가 단일 소스. `resultsLinkMail`(③ 교사가 [결과지 받기]를 눌렀을 때 나가는 결과 페이지 링크, 14일)도 여기. ⚠️ 승인 안내는 **3채널**(이 파일의 HTML · `format.ts`의 `approvalNoticeText` 평문 · 관리자 화면 [안내 문구 복사])이므로 한쪽만 고치지 말 것 |
