...
| `consent.ts` | 고지·동의 문구의 단일 소스. 법정대리인 동의 확인 기록(개인정보보호법 제22조의2)의 근거 문구 + `RETENTION_LABEL`(보관 기간 — 화면과 가정통신문 서면 양식이 공유해야 하는 값) + 신청 화면(`/apply`)의 `SURVEY_NOTICE`·`APPLY_CHECKS`. 신청 화면 문구가 화면이 아니라 여기 있는 이유는 ①서면 양식과 갈리면 안 되고 ②node 환경 테스트가 문구 존재를 핀할 수 있어야 하기 때문이다(화면 렌더 테스트는 두지 않는 관례) |
