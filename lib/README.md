...
| `survey-flow.ts` | 페이지 게이팅. 중단 규칙(①·②)과 현장 채점은 담당자 확정(2026-08-13)으로 전면 폐기됐다 — 검사 화면은 수집만 하고 판정·채점은 관리자 결과지가 한다. `visiblePages`(연습 실시 여부만 반영)·`canAdvance`(문항 완료 여부로 [다음] 활성화 판단) |
