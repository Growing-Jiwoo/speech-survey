 ...
 *
 *  ⚠️ 명단 드롭다운 도입(Task 12, 2026-08-18)으로 응답 payload가 바뀌었다 — childNo 없이
 *  부르면 이제 그 학급 전원의 실명·성별·생년월일이 담긴 명단을 돌려준다. 즉 코드 추측 한 번이
 *  성공했을 때의 대가가 "담임 연락처 + 아동 한 명의 검사 상태"에서 "학급 전체 명단"으로
 *  커졌다. 위 상한값(코드 공간 크기가 진짜 방어라는 계산)은 그대로 맞지만, 다음에 이 값을
 *  조정할 사람은 "적중 한 번의 대가가 이만큼 커졌다"를 감안해야 한다. */
