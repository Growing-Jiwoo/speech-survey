 ...
 *  `fromRoster: z.undefined().optional()` — `fromRoster: true`가 이 스키마를 통과하는 일을
 *  원천 차단한다(`.optional()`이 없으면 이 필드를 아예 안 보내는 보통의 직접 입력 요청조차
 *  키가 없다는 이유로 거부된다 — z.undefined()는 "값이 undefined"가 아니라 "키가 존재하고
 *  그 값이 undefined"를 요구한다). 이게 없으면 z.union의 멤버 순서가 "명단 모드는 이름·
...
/** 두 모드의 유니온. z.union은 순서대로 시도해 처음 통과하는 스키마를 쓴다 — 명단 모드를
 *  먼저 둔 것 자체는 우연이 아니지만(정상 요청이 어느 쪽이든 한 번에 맞는 스키마로 붙게),
 *  **불변식(신원 위조 방지)이 이 순서에 기대면 안 된다** — 그래서 `sessionCreateDirectSchema`에
 *  `fromRoster: z.undefined()`를 박아 뒀다: `fromRoster: true`인 바디는 순서를 뒤집어도
 *  direct 스키마를 절대 통과할 수 없다.
 *  두 z.object 모두 기본이 "모르는 키는 벗긴다"라, fromRoster:true와 함께
 *  name/gender/birthYmd를 얹어 보내는 요청(클라이언트가 신원을 같이 보냈다고 착각하는 상황)은
 *  명단 스키마가 통과해 그 필드들을 조용히 벗겨낸다. 이는 트랩이 아니라 의도다: 명단 모드가
 *  서버 신뢰 값만 쓴다는 계약을 정확히 반영한다(라우트가 어차피 그 필드를 읽지 않으므로
 *  .strict()로 400 처리해도 실질은 같지만, 400은 "형식 오류"를 뜻하고 여기 실제 상황은
 *  "무시됐을 뿐 요청은 유효하다"이므로 200 + 조용한 무시 쪽이 상태 코드의 의미에 더 맞는다). */
