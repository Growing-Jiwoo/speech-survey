...
// proxy(옛 middleware)는 /admin 전체의 유일한 인증 계층이다(라우트들은 "인증은 middleware가 담당" 전제).
...
describe('proxy — 인증 실패', () => {
  it('토큰 없는 /admin 페이지는 로그인으로 리다이렉트', async () => {
    const res = await proxy(mkReq('/admin'))
    expect(res.status).toBe(307)
    expect(res.headers.get('location')).toContain('/admin/login')
  })

  it('토큰 없는 /api/admin/* 는 401 JSON (리다이렉트 아님)', async () => {
    const res = await proxy(mkReq('/api/admin/sessions'))
  ...
  it('다른 시크릿으로 서명된 토큰 거부', async () => {
    const res = await proxy(mkReq('/admin', await createToken('other-secret')))
    expect(res.status).toBe(307)
  })

  it('만료된 토큰 거부', async () => {
    const res = await proxy(mkReq('/admin', await createToken(SECRET, -1000)))
    expect(res.status).toBe(307)
  })
  ...
  it('[REGRESSION] SESSION_SECRET 미설정 시 fail-closed — 어떤 토큰도 통과 불가', async () => {
    ...
    const res = await proxy(mkReq('/admin', token))
    expect(res.status).toBe(307)
  })
})

describe('proxy — 인증 성공', () => {
  it('유효 토큰이면 /admin·/api/admin/* 모두 통과', async () => {
    const token = await createToken(SECRET)
    expect(passed(await proxy(mkReq('/admin', token)))).toBe(true)
