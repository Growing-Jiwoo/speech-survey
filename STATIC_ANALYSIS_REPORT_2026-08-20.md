...
**담당자 질문은 4건 남았다** — 3번(중단 규칙 ② 폐기)은 담당자 확정, 6번(진행률 표기)은
사용자 확정으로 2026-08-22에 해소됐다. 나머지는 본문 8절 참고.
...
3. ~~**중단 규칙 ②(쓰기 중단) 폐기 재확인**~~ — ✅ **해소(2026-08-22).** 담당자가 「즉시 중단 기능 제거」가 규칙 ②까지 포함한다고 확정했다. 1번 문항이 0점이어도 나머지 쓰기 문항을 전부 실시하는 현재 동작이 맞다([lib/survey-flow.ts:5-14](lib/survey-flow.ts#L5)). 회신 원문은 저장소에 없다(사용자 전달)
4. **메일·신청 화면 문구** — ⏳ **초안은 벗었다(2026-08-22, 사용자 확정).** 교사가 실제로 묻는 것(소요 시간·준비물·코드 분실)에 답하도록 다시 썼고, 「검사 절차 안내를 확인했습니다」가 가리킬 안내를 화면에 만들었다(이전 판은 확인할 대상이 없는 가짜 동의였다). **여전히 연구윤리 검토본 대기** — 검토본이 오면 `SURVEY_NOTICE`·`APPLY_CHECKS`와 `lib/mail.ts` 문구 절만 교체([lib/mail.ts](lib/mail.ts), [app/apply/page.tsx](app/apply/page.tsx))
5. **보존 기한** — "목적 달성 시 지체 없이 파기" 일반 원칙을 구체 기한으로(운영 주체 결정, [lib/consent.ts:13-21](lib/consent.ts#L13))
6. ~~**진행률 표기 "문항 N/M"**~~ — ✅ **해소(2026-08-22, 사용자 확정).** 현행 유지. "문항"은 채점 문항이 아니라 **시행 단위**를 뜻한다 — 한 페이지가 녹음 1개·제한시간 1개이고, 녹음을 낱말별로 쪼갤 수 없어 셀 수 있는 최소 단위가 페이지다([components/ProgressBar.tsx](components/ProgressBar.tsx))
