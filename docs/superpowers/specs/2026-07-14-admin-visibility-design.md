...
5. **테이블**
   - 컬럼: 이름 · 학교 · 학년/반 · 시작 · 진행률 · 체크 · 상태. (생년월일 컬럼은 결과지로 이동 판단 —
     목록 스캔에 불필요한 밀도라 제거. 검색·식별은 이름+학교+학년/반으로 충분.)
...
- 모든 집계·필터·정렬은 클라이언트 순수 함수. 상태는 URL에 싣지 않음(새로고침 시 초기화 허용 —
  수백 건 규모에서 재필터 비용 무시 가능, 딥링크 요구 없음).
