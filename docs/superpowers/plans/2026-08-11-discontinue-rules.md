...
**Task 7부터 계획대로 이어간다.** Task 1·2+3·4·5·6 모두 구현·스펙 리뷰·품질 리뷰 전부
완료됐다(위 참고 절). Task 6이 끝나면서 "🚫 배포 금지" 절의 검사 화면 배너 악화가 풀렸다 —
남은 건 관리자 ScoreBand(Task 7)뿐이다.

**Task 7 사전 대조 완료**: Task 5의 표-산문 불일치 전례를 우려해 Task 7 절의 `ResultSheet`
배선을 스펙 관리자 화면 표와 미리 대조했다 — `cells` 배열에서 무의미에만 `na: disc.wordReading`
가 붙고 의미는 정상 표시로 남는다(의미 소계는 정당하게 채점된 값이므로 계속 보여야 한다).
같은 실수는 없다. Task 7 착수 시 다시 볼 필요 없음.
