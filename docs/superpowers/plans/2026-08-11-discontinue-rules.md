...
| 5 stamp-sheet | ✅ `ad6836f`+`a31a60e`+`296a690` | ✅ 통과 | ✅ 승인 (3라운드 — 아래 참고) |
| 6 검사 화면 모달 | ✅ `430eb7f`+`95b67e7`+`79f590a` | ✅ 통과 | ✅ 승인 (2라운드 — 아래 참고) |
| 7 관리자 결과지 | ✅ `0fe040e`+`9685aae` | ✅ 통과 | ✅ 승인 (2라운드 — 아래 참고) |
| 8 마이그레이션 013 | ✅ 파일·커밋 `175d490` | — (SQL 주석, 리뷰 생략) | ⏸ **실 DB 미적용** — 아래 참고 |
| 9 | ⏸ 미착수 | | |

### Task 8 — 파일은 준비됐지만 실 Supabase 적용은 이 세션 도구로 불가능

`013_discontinued_comment.sql`은 작성·커밋·푸시됐다. **다만 실 DB에는 적용 안 됐다** —
`COMMENT ON COLUMN`은 PostgREST(REST API)로 실행할 수 없는 DDL이고, 이 환경엔 직접 postgres
연결 문자열도 `psql`도 없다(`.env.local`에는 `SUPABASE_URL`·`SUPABASE_SERVICE_ROLE_KEY`만
있다 — 읽기 쿼리에는 충분하지만 임의 SQL 실행에는 못 쓴다).

**사용자가 직접 적용해야 한다** — Supabase 대시보드의 SQL 편집기에 파일 내용을 붙여넣고
실행하면 된다. 스키마 변경이 없고 컬럼 설명(주석)만 바뀌므로 롤백 부담·데이터 위험이 없다.
적용 여부는 앱 동작에 영향을 주지 않는다(주석은 애플리케이션 코드가 읽지 않는다) — 배포를
막는 게이트가 아니다.

### Task 7 리뷰에서 실제로 잡힌 것 (참고) — Task 5와 같은 근본 원인

스펙 리뷰는 6개 파일 전부 계획과 문자 단위로 일치함을 확인하고 통과시켰다. 문제는 **계획
자체**에 있었다 — 스펙 문서 106~116행이 "판정 기준은 플래그가 아니라 응답 부재다. 표시와
완료 판정은 서로 다른 근거를 쓴다"고 명시했는데, Task 7 계획 절이 이 원칙을 `disc.wordReading`
같은 플래그로 대체해 옮겼다. **Task 5(PDF)에서 났던 것과 정확히 같은 종류의 실수**(스펙
문서의 표/원칙과 계획의 구현 절이 어긋남)가 재발했다.

품질 리뷰가 실측으로 문제를 확인했다 — 옛 규칙 세션이나 관리자의 사후 정정으로 무의미 marks가
실제로 존재하는데도 플래그 하나로 "미실시"를 표시하면, 소계 계산(`r.wordReading` 등)은
`discontinued`와 무관하게 그 marks를 그대로 더하므로 **"의미 3/7 · 무의미 미실시 · 총점
8/14"처럼 셀의 합과 총점이 어긋나는 화면**이 나온다. 소급 데이터 없이도 정상 채점 흐름
(검사자가 무의미를 실시·녹음 → 관리자가 나중에 정정)에서 도달한다. 쓰기 섹션에는 `na` 자체가
누락돼 ② 세션에서 "무의미 점수 0/5"가 그대로 찍히는 문제도 있었다.

`components/admin/sheet/PageAudio.tsx`의 `attempts.length === 0` 검사("데이터 있으면 재생기
우선")가 이미 정확한 패턴이었다 — 같은 원칙을 `ResultSheet.tsx`에 헬퍼 3개
(`nonsenseReadNA`·`sentenceReadNA`·`nonsenseWriteNA` — "플래그 && 응답 없음")로 확장해
해소했다(`9685aae`). **`Subtotal`의 `discontinued` prop과 `ScoreBand`의 판정은 그대로 규칙
플래그(`disc.*`) 기준으로 남겼다** — 배지·Pass/Fail 근거는 규칙이 맞고, 이번에 바뀐 건 셀
표시·입력 잠금뿐이다(스펙의 "표시와 완료 판정은 서로 다른 근거" 그대로).

**교훈 재확인**: 스펙 문서에 원칙과 구체적 구현 절이 따로 있으면 반드시 원칙 쪽과도 대조할
것 — 이걸로 이 브랜치에서 같은 패턴이 두 번(Task 5, Task 7) 났다. Task 9 최종 검증에서
남은 스펙 절(마이그레이션 013 관련)도 이 관점으로 한 번 더 훑을 것.

별건: 리뷰 중 `tests/stamp-sheet.test.ts`가 PDF 메타데이터 타임스탬프 때문에 플레이키하다는
것도 발견됐다 — 별도 작업 칩으로 이미 등록해뒀다(`task_b2133cec`, 이 브랜치 범위 밖).

### Task 6 리뷰에서 실제로 잡힌 것 (참고)

구현 자체(모달 2개, 발화 조건, 문구 10항목)는 스펙 리뷰에서 문자 단위로 정확했다. 구현자가
스스로 발견해 즉시 고친 것 — 검토 화면(`from=review`) 재진입 시 모달이 "쓰기 과제로 이동"을
말하고 실제로는 `/review`로 복귀해 목적지가 거짓이었다. 기존 `skipping`·`showIntro`의
`!fromReview` 억제 관례를 그대로 적용해 해소(`95b67e7`).

품질 리뷰에서 정확성 버그는 0건이었으나 구조 문제 2건을 같은 라운드에서 처리했다(`79f590a`):
1. `MarkPage.tsx` 머리 주석이 옛 규칙("쓰기 미실시")을 그대로 두고 있어 같은 파일의 새 배너와
   정면 모순됐다 — 계획서가 "두 군데 틀렸다"고 센 것 중 세 번째 지점을 놓친 것.
2. 규칙 ② 발화 로직이 두 지점(문항별 입력·일괄 버튼)에 복제됐고, 그중 하나가
   `f.writingItems[0]` 참조로 "어느 문항이 판정을 정하는가"라는 `survey-flow`의 내부 지식을
   컴포넌트로 새게 했다 — 이 브랜치 내내 지켜온 "판정 로직은 한 곳" 원칙의 유일한 균열이었다.
   `maybeWritingCeiling(next)` 하나로 합쳐 순 -6줄로 해소.

이 태스크는 컴포넌트 테스트가 없어 tsc/lint만으로 검증했다 — 두 리뷰 모두 코드를 손으로
추적해 5~9가지 엣지 케이스(모달 1회 발화, 상태 스테일, `paused`↔모달 교차, `busy` 충돌 등)를
직접 검증했다.

### Task 5 리뷰에서 실제로 잡힌 것 (참고 — 3라운드나 간 이유)

1차 구현(`ad6836f`)은 계획과 정확히 일치했다. 문제는 **계획의 원천인 스펙 문서 자체**에
있었다 — 210~211행 표는 "① 세션에서 의미 점수 `/7`은 찍는다"인데, 222~224행 "### 구현" 절이
그걸 무시하고 낱말 해독 소계 3칸을 한 조건으로 뭉쳐 적어 놨다. 계획이 그 잘못된 절을 그대로
옮겼고, 구현자는 계획을 정확히 따랐을 뿐이다. 스펙 문서를 먼저 고치고(표와 일치하도록) 코드를
다시 고쳤다(`a31a60e`) — 의미 점수만 별도 조건으로 분리.

같은 라운드에서 테스트도 문제였다 — 바이트 크기 비교라 조건 4개 중 1개만 잡았다(크기를
지배하는 건 그리기 항목 수가 아니라 폰트 서브셋 글리프 집합). `pdfjs-dist`(이미 devDependency)로
점수 칸 좌표를 직접 읽는 `stampedAt` 헬퍼로 교체했다.

3차(`296a690`)는 재검증에서 나온 새 지적이다 — 소계는 고쳤지만 **문항별 표시**(무의미 O/X,
문장 문항별, 쓰기 2~5번)는 여전히 조건 없이 찍히고 있었다. "실시 안 한 문항엔 값이 없다"는
전제(스펙 구현 절이 명시)가 Task 7(입력 잠금) 전인 지금은 참이 아니다 — 관리자가 여전히 그
칸들을 채점할 수 있고, 채점하면 그대로 인쇄된다. `requiredWritingCodes`(기존 함수 재사용)로
쓰기를, `discontinued.wordReading`/`sentenceReading`으로 나머지 둘을 걸렀다. "관리자가 미실시
문항에 값을 넣어도 인쇄되지 않는다"는 적대적 테스트(`strayNonsense`)와 "①만 걸리면 쓰기는
그대로 다 찍힌다"는 정상 경로 테스트를 함께 추가해 회귀 방지선을 세웠다.

**교훈**: 스펙 문서에 표와 산문이 같은 내용을 두 번 적으면 어긋날 수 있다. Task 7에서 같은
스펙의 관리자 화면 표를 구현할 때 이 전례를 참고할 것.
...
- **스펙 리뷰**: 계획 문서의 픽스처 자체에 함정이 있었다 — `can_write: false`를 10개 전부에 줘도 `writingCeilingHit`이 0번 문항만 보므로 분모가 1로 줄고, "쓰기 전체로 완료" 테스트가 실제로는 다른 것(①+②)을 증명하고 있었다. `131b35b`에서 교정.
- **품질 리뷰**: 실 Supabase 세션 `e0ac9d94`(제출 완료, `discontinued_at: null`, 낱말 쓰기 10문항 응답, `ww01=false`)가 새 규칙 ②에 **소급 적용돼 분자(10)가 분모(1)를 넘는** 실제 사례였다. `SessionTable.tsx`의 진행 바 폭 계산이 1000%까지 나오고, 정렬 비율도 1을 넘어 다 끝난 세션을 밀어냈다. `e5e0b9b`에서 두 곳 모두 `Math.min` 클램프 + 회귀 테스트로 고정.
- Minor 4건(recordingPages 필터 섹션 가드, 주석-코드 모순, 테스트 이름 정확성 1건, 죽은 가드)은 **의도적으로 스킵** — Task 7에서 같은 파일을 다시 열 때 처리.

### ▶ 재개할 때 첫 할 일

**Task 9(최종 검증)부터 이어간다.** Task 1~8 전부 완료됐다. Task 8의 실 DB 적용만 사용자
몫으로 남아 있고(위 참고 절), 이건 Task 9 진행을 막지 않는다(주석 변경은 앱 동작과 무관).
...
## ✅ Task 9 완료 (`0fce8c8`까지)

**Step 1** — `npx tsc --noEmit && npm run lint && TZ=UTC npx vitest run && npm run build` 전부 통과.
486 tests, 워킹트리 깨끗.

**Step 2 (검사 화면)** — G1 신규 세션으로 전 과정 실제 조작:
- 의미 낱말 1·2·3번 X → 4~7번 잠기지 않고 채점 가능 확인. [다음] → 모달 ① →
  [다시 채점]으로 취소(값 유지 확인) → 재확인 → [쓰기 과제로 이동] → 무의미·문장
  페이지 없이 바로 쓰기 섹션 도착(진행률 10→5로 자동 축소, `visiblePages` 확인).
- 쓰기 1번 "아니오" → 입력 즉시 모달 ② → [다시 입력] → 1번 "예"로 바꾸면 2~10번
  풀림 확인 → 다시 "아니오" → 모달 재발화 안 함(1회 발화, `writingModalSeen` 확인)
  → [다음]으로 체크리스트 진입.
- 진행률 바 분모가 페이지 감소를 정확히 따라감(3/5 → 4/5 → 5/5).

**부수 발견 — `app/review/page.tsx`의 배너 버그(`0fce8c8`)**: 검토 화면이 "중단
규칙에 따라 문장 읽기유창성·낱말 쓰기는 생략되었습니다"라는 **옛 규칙 문구**를
그대로 띄우고 있었다. 새 규칙에서 쓰기는 항상 실시하므로 방금 쓰기 과제를 마친
검사자에게 거짓을 말하는 상태였다. **이 파일은 Task 1~8 어느 목록에도 없었다** —
계획이 놓친 지점을 실제 브라우저 워크스루가 잡았다. `visiblePages`는 규칙①로만
페이지를 줄이므로 "문장 읽기유창성이 생략되었습니다"로 단순화해 고쳤다.

**Step 3 (관리자 화면·PDF)** — 사용자가 로그인. ①+② 동시 발동 세션으로 확인:
- 목록 진행률 "녹음 0/1 · 쓰기 1/1" — 옛 분모(6/10)가 아니라 축소된 분모로 정확히 표시.
- `discontinued_at`이 제출 시점에 정확히 기록됨(DB 직접 조회로 확인).
- ScoreBand 세 카드 전부 `중단` 배지 + 사유 문구("의미 낱말 첫 3개 연속 오반응 ·
  실시분 4점" 등). 무의미 그룹 `미실시` 배지 + O/X 잠금. 소계 "의미 점수 4/7 ·
  무의미 점수 미실시 · 총 점수 4/14 · 중단" — 총점이 의미 점수와 정확히 일치(자기
  모순 없음, Important 2에서 고친 지점이 실측으로 확인됨). 문장 4개 전부 `미실시`
  배지 + 입력 잠금(disabled 확인). BadgeLegend·안내 문구 전부 정확.
- PDF를 fetch+base64로 받아 `pdftoppm`으로 렌더링해 육안 확인 — 스펙 표 6행과
  완전히 일치: 의미 O/X 7개 + 의미 점수 4/7 **찍힘**, 무의미 O/X·소계·총점 **빈칸**,
  문장 4개 전부 **빈칸**(문항·총점), 쓰기 1번(X) **찍힘**, 쓰기 2~5번·소계·총점
  **빈칸**, 체크리스트 **찍힘**. Task 5의 3라운드 수정이 실물 PDF에서 전부 검증됨.

**발견했지만 고치지 않은 것 — G1 쓰기 "의미 점수" 부분 절단 케이스**:
규칙②가 걸린 G1 세션의 관리자 화면에서 "의미 점수 **0** / 5"가 확정값으로
보인다. 실제로는 1번(ww01, 의미 그룹)만 실시되고 2~5번(역시 의미 그룹)은
미실시인데, Task 7이 고친 `na` 처리는 **무의미 그룹 전체**에만 적용돼 있어
이 케이스(중단 지점이 의미 그룹 **안**에서 발생)를 못 덮는다. PDF는 이 경우
소계 3칸(의미·무의미·총점)을 전부 안 찍는데(discontinued.writing 하나로
묶여 있으므로), 화면은 무의미만 가리고 의미는 그대로 노출해 PDF와 화면이
갈린다. G2는 이 소계 자체가 없어(문장 쓰기는 총점만 표시) 영향 없음 —
**G1에만 있는 좁은 케이스.** 표시값 자체가 틀린 숫자는 아니고(0점은 실제
계산값), "미실시분이 확정값처럼 보인다"는 이 태스크의 핵심 원칙과 미묘하게
어긋나는 정도라 배포를 막을 사안은 아니라고 판단해 별도 후속으로 남긴다.

**Step 4** — 테스트 세션(테스트중단일) 삭제 확인(DB 직접 조회로 빈 배열 확인).
기존 테스트 세션 10건 무변경.

## 별건 — 플레이키 테스트 제안 검증 (`task_b2133cec`)

사용자가 제시한 제안("`PDFDocument.load(srcPdf, { updateMetadata: false })`로
`stamp-sheet.ts`의 바이트 재현성 확보")을 코드·실측으로 검증했다:
- 제안의 타이밍 설명은 부정확함(`save()` 아니라 `load()` 생성자 시점) — 하지만
  결론(같은 입력도 두 번 로드하면 ModDate가 벽시계 차이로 달라짐)은 정확.
- `node`로 직접 재현: `updateMetadata` 기본값(true)에서 1.2초 간격 두 번 호출 →
  바이트 불일치. `updateMetadata: false`로 → 완전히 동일한 바이트.
- **타당함, 적용 권장.** 부작용 없음(Producer/CreationDate 메타데이터만 안 찍힘).
  이 브랜치 범위 밖이라 반영은 안 함 — 칩(`task_b2133cec`)으로 남아 있음.
